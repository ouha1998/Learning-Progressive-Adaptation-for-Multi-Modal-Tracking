...
from lib.test.tracker.patrack import PATrack
import lib.test.parameter.patrack as rgbt_adapter_params
...
def genConfig(seq_path, set_type):
    if set_type == 'RGBT234':
        ...
        ############################################  have to refine #############################################
        RGB_img_list = sorted([seq_path + '/v/' + p for p in os.listdir(seq_path + '/v') ])
        T_img_list = sorted([seq_path + '/i/' + p for p in os.listdir(seq_path + '/i') ])
...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, num_gpu=1, epoch=300, debug=0, script_name='adapter'):
    seq_txt = seq_name
    save_name = '{}_ep{}'.format(yaml_name)
    print(save_name)
    ...
    if script_name == 'patrack':
        params = rgbt_adapter_params.parameters(yaml_name, epoch)
        mmtrack = PATrack(params)  # "GTOT" # dataset_name
        tracker = PATrack_RGBT(tracker=mmtrack)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--script_name', type=str, default='', help='Name of tracking method(ostrack, adapter, ftuning).')
    parser.add_argument('--yaml_name', type=str, default='rgbt', help='Name of tracking method.')  # vitb_256_mae_ce_32x4_ep300 vitb_256_mae_ce_32x4_ep60_adapter_i32v21_onlylasher_rgbt
    ...
    if dataset_name == 'GTOT':
        ...
        seq_home = ''
        with open(join(seq_home, 'testingsetList.txt'), 'r') as f:
