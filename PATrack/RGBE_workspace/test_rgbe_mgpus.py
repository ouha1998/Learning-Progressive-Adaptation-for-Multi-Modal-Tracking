...
import lib.test.parameter.patrack as rgbe_adapter_params
...
def genConfig(seq_path, set_type):
    ...
    if set_type == 'COESOT':
        filename1 = seq_path.split("/")[-1] + '_aps/'
        filename2 = seq_path.split("/")[-1] + '_dvs/'
        path1 = os.path.join(seq_path, filename1)
        path2 = os.path.join(seq_path, filename2)
        RGB_img_list = sorted([path1 + p for p in os.listdir(path1) ])
        E_img_list = sorted([path2 + p for p in os.listdir(path2)])
...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, num_gpu=1, epoch=60, debug=0, script_name='prompt'):
    ...
    params = rgbe_adapter_params.parameters(yaml_name, epoch)
    mmtrack = PATrack(params)  # "GTOT" # dataset_name
    tracker = PATrack_RGBE(tracker=mmtrack)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--yaml_name', type=str, default='rgbe', help='Name of tracking method.')
